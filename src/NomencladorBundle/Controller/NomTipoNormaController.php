<?php

namespace NomencladorBundle\Controller;

use NomencladorBundle\Util\Util;
use ReporteBundle\Util\GenerarExcel;
use Symfony\Component\HttpFoundation\JsonResponse;
use Symfony\Component\HttpFoundation\Request;
use Symfony\Bundle\FrameworkBundle\Controller\Controller;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Method;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Route;
use NomencladorBundle\Entity\NomTipoNorma;
use PHPExcel;
use Sensio\Bundle\FrameworkExtraBundle\Configuration\Security;

/**
 * NomTipoNorma controller.
 *
 * @Route("/nomtiponorma.yml")
 */
class NomTipoNormaController extends Controller
{
    /**
     * Lists all NomTipoNorma entities.
     *
     */
    /**
     * @Security("is_granted('ROLE_LISTAR_TIPONORMACONSUMO')")
     */
    public function indexAction($remember)
    {
        $em = $this->getDoctrine()->getManager();

        $nomTipoNormas = $em->getRepository('NomencladorBundle:NomTipoNorma')->findAll();

        return $this->render('NomencladorBundle:nomtiponorma:index.html.twig', array(
            'nomTipoNormas' => $nomTipoNormas,
            'remember'=>$remember
        ));
    }
    /**
     * @Security("is_granted('ROLE_LISTAR_TIPONORMACONSUMO')")
     */
    public function listarAction(Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $st = $request->query->get('start') ? $request->query->get('start') : 0;
        $lm = $request->query->get('limit') ? $request->query->get('limit') : 10;
        $where = '1=1';
        $filters_raw = $request->query->get('filters');
        if ($filters_raw) {
            if (!in_array("activo:0", $filters_raw) && !in_array("activo:1", $filters_raw) && !in_array("activo:2", $filters_raw)) {
                $where .= " AND g.activo = 1";
            }
            foreach ($filters_raw as $f) {
                $sp = explode(':', $f);
                if ($sp[0] != "activo") {
                    $alias = Util::getSlug($sp[1]);
                    $where .= 'AND g.' . $sp[0] . ' LIKE \'%' . $alias . '%\'';
                } elseif ($sp[0] == "activo") {
                    if ($sp[1] < 2) {
                        $where .= 'AND g.activo = ' . $sp[1];
                    }
                }
            }
        }
        $dql = 'SELECT g
            FROM NomencladorBundle:NomTipoNorma g
            WHERE ' . $where . ' ORDER BY g.codigo ';
        $consulta = $em->createQuery($dql);
        $total = count($consulta->getResult());
        $consulta->setFirstResult($st);
        $consulta->setMaxResults($lm);
        $nom = $consulta->getResult();
        $for = array();

        foreach ($nom as $pr) {
            $res = array();
            $res[] = $pr->getId();
            $res[] = $pr->getCodigo();
            $res[] = $pr->getNombre();
            $res[] = $pr->getActivo();
            $for[] = $res;
        }
        if ($total > 0) {
            return new JsonResponse(array('data' => $for, 'total' => $total));
        } else {
            return new JsonResponse(array('data' => [], 'total' => 0));
        }
    }


    /**
     * Creates a new NomTipoNorma entity.
     *
     */
    /**
     * @Security("is_granted('ROLE_ADICIONAR_TIPONORMACONSUMO')")
     */
    public function newAction(Request $request)
    {
        $nomTipoNorma = new NomTipoNorma();
        $form = $this->createForm('NomencladorBundle\Form\NomTipoNormaType', $nomTipoNorma);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $nomTipoNorma->setActivo(1);
            $em->persist($nomTipoNorma);
            $em->flush();
            $this->addFlash('success', 'Se ha insertado el elemento correctamente.');
            if ($form->get('aceptar')->isClicked())
                return $this->redirectToRoute('nomtiponorma_index',array('remember'=> 1));
            else
                return $this->redirectToRoute('nomtiponorma_new');
        }

        return $this->render('NomencladorBundle:nomtiponorma:new.html.twig', array(
            'nomTipoNorma' => $nomTipoNorma,
            'form' => $form->createView(),
            'accion' => 'Adicionar'
        ));
    }

    /**
     * Displays a form to edit an existing NomTipoNorma entity.
     *
     */
    /**
     * @Security("is_granted('ROLE_MODIFICAR_TIPONORMACONSUMO')")
     */
    public function editAction(Request $request, NomTipoNorma $nomTipoNorma)
    {        
        $editForm = $this->createForm('NomencladorBundle\Form\NomTipoNormaType', $nomTipoNorma);
        $editForm->handleRequest($request);

        if ($editForm->isSubmitted() && $editForm->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $em->persist($nomTipoNorma);
            $em->flush();
            return $this->redirectToRoute('nomtiponorma_index', array('remember'=> 1));
        }

        return $this->render('NomencladorBundle:nomtiponorma:new.html.twig', array(
            'nomTipoNorma' => $nomTipoNorma,
            'form' => $editForm->createView(),
            'accion' => 'Editar'
        ));
    }

    /**
     * @Security("is_granted('ROLE_ELIMINAR_TIPONORMACONSUMO')")
     */
    public function eliminarAction(Request $request)
    {
        $tiponorma = $request->request->get('id');

        $params['campoId'] = "id";
        $params['tabla'] = "NomTipoNorma";
        $params['valor'] = $tiponorma;
        if (!is_array($params['valor']))
            $params['valor'] = [$params['valor']];

        if (count($params['valor']) > 1) {
            $params['nomenclador'] = "los elementos seleccionados.";
        } else if (count($params['valor']) == 1) {
            $params['nomenclador'] = "el elemento seleccionado.";
        }
        $result = $this->get('nomencladores')->eliminarObjEntidad($params);

        if ($result['enUso'] && count($params['valor']) > 1) {
            $result['mensaje'] = 'Se eliminaron solo los Tipos de Norma de consumo que no están en uso. Verifique en el Nomenclador de Norma de Consumo y Parte Diario de Consumo de Materia Prima.';
        } else if ($result['enUso'] && count($params['valor']) == 1) {
            $result['mensaje'] = 'No se pudo eliminar el Tipo de Norma de consumo seleccionado ya que está en uso. Verifique en el Nomenclador de Norma de Consumo y Parte Diario de Consumo de Materia Prima.';
        }

        return new JsonResponse(array('respuesta' => $result['msg'], 'mensaje' => $result['mensaje']));

    }

    /**
     * @Security("is_granted('ROLE_LISTAR_TIPONORMACONSUMO')")
     */
    public function setActivoAction(Request $request)
    {
        $elementos = $request->request->get('id');
        $accion = $request->request->get('accion');
        $em = $this->getDoctrine()->getManager();
        for ($i = 0; $i < count($elementos); $i++) {
            $tipo = $em->getRepository('NomencladorBundle:NomTipoNorma')->find($elementos[$i]);
            $accion == 'btn_activar' ? $estado = true : $estado = false;
            $tipo->setActivo($estado);
        }
        $em->flush();
        return new JsonResponse(array('respuesta' => 'exito'));
    }

    /**
     * @Security("is_granted('ROLE_LISTAR_TIPONORMACONSUMO')")
     */
    public function exportarAction(Request $request)
    {
        $filtroCodigo = $request->query->get("codigo");
        $filtroNombre = $request->query->get("nombre");

        $em = $this->getDoctrine()->getManager();
        $data = $em->getRepository('NomencladorBundle:NomTipoNorma')->listarDelExportarTipoNorma($filtroCodigo, $filtroNombre);
        $datosEmp = $em->getRepository('AdminBundle:DatConfig')->obtenerDatosEmpresa();

        // Crea un nuevo objeto PHPExcel
        $objPHPExcel = new PHPExcel();
        $objPHPExcel->getProperties()
            ->setCreator('Coppelia')
            ->setTitle('Tipo norma de consumo')
            ->setLastModifiedBy('Coppelia')
            ->setDescription('Nomenclador concepto sistema coppelia')
            ->setSubject('Office 2007 XLSX Test Document')
            ->setKeywords('exportar nomenclador tipos de norma de consumo')
            ->setCategory('exportar');
        // Agregar Informacion
        $objPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(10);
        $objPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(40);
        $celda = $objPHPExcel->setActiveSheetIndex(0);
        $celda->mergeCells('A2:B2');
        $celda->setCellValue('A2', count($datosEmp) > 0 ? 'Empresa: ' . $datosEmp[0]->getNombreEntidad() : "Empresa: ");
        $celda->mergeCells('A3:B3');
        $celda->setCellValue('A3', 'Reporte: Nomenclador Tipos de Norma de consumo');

        $celda->setCellValue('A5', 'Código')
            ->setCellValue('B5', 'Nombre')
            ->setCellValue('C5', 'Activo');
        $celda->setTitle('Tipos de Norma de consumo');
        $celda->fromArray($data, '', 'A6');
        $objPHPExcel->setActiveSheetIndex(0);
        $header = 'a5:c' . (count($data) + 5);
        $cabecera = 'a5:c5';
        $encabezado = 'A2:D2';
        $encabezado1 = 'A3:D3';
        $style = array(
            /*'font' => array('bold' => true,),*/
            'alignment' => array('horizontal' => \PHPExcel_Style_Alignment::HORIZONTAL_LEFT,),
            'borders' => array('outline' => array('style' => \PHPExcel_Style_Border::BORDER_THICK, 'color' => array('argb' => 'FF000000'))),
        );
        $style2 = array(
            'font' => array('bold' => true,),
            'borders' => array('outline' => array('style' => \PHPExcel_Style_Border::BORDER_THICK, 'color' => array('argb' => 'FF000000'))),
        );
        $style3 = array(
            'font' => array('bold' => true)
        );
        $celda->getStyle($header)->applyFromArray($style);
        $celda->getStyle($cabecera)->applyFromArray($style2);
        $celda->getStyle($encabezado)->applyFromArray($style3);
        $celda->getStyle($encabezado1)->applyFromArray($style3);
        $objPHPExcel->setActiveSheetIndex(0);
        $celda->getStyle($cabecera)->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID)->getStartColor()->setRGB('54ae86');
        $params['nameArchivo'] = "tipo_norma_consumo";
        $objGenerarExcel = new GenerarExcel();
        $objGenerarExcel->salidaExportarExcel($objPHPExcel, $params);

    }
}
